...
SERVER_COMMAND = PROJECT_ROOT / ".venv" / "bin" / "agent-substrate-mcp"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dogfood the Agent Substrate MCP server over stdio.")
    parser.add_argument(
        "--state-dir",
        type=Path,
        default=None,
...
async def run_dogfood(state_dir: Path, repo_path: Path) -> dict[str, Any]:
    env = dict(os.environ)
    env["AGENT_SUBSTRATE_HOME"] = str(state_dir)
        ...
        async with ClientSession(read_stream, write_stream) as session:
            ...
            domain = await call(
                ...
                "domain_create",
                {
                    ...
                    "repos": [str(repo_path)],
                    "tags": ["python", "mcp", "agent-substrate"],
                },
            )
...
async def main_async() -> int:
    ...
    if args.state_dir is None:
        with tempfile.TemporaryDirectory(prefix="agent-substrate-dogfood-") as tmp:
